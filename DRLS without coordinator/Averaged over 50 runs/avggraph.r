...
#Initializing matrices for av MSE calculations
avgMSE_d<-matrix(0,sample,N)
avgMSE_c<-matrix(0,sample,1)
avgMSE<-matrix(0,sample,1)

#loop for running the RLS for 50 times
...
avgMSE<-avgMSE+MSE
avgMSE_c<-avgMSE_c+avMSE_d
avgMSE_d<-avgMSE_d+MSE_d
}

#calculating averages

avgMSE<-avgMSE/50
avgMSE_c<-avgMSE_c/50
avgMSE_d<-avgMSE_d/50


#plotting MSE curves for a few agents
par(mfrow=c(2,2))

plot(avgMSE_d[,1], col="red",xlab="Iterations",ylab="Squared Error in dB", type="b", main="Agent 1",pch=16)
plot(avgMSE_d[,7], col="red",xlab="Iterations",ylab="Squared Error in dB", type="b", main="Agent 7",pch=16)
plot(avgMSE_d[,16], col="red",xlab="Iterations",ylab="Squared Error in dB", type="b", main="Agent 16",pch=16)
plot(avgMSE_d[,maxneighnode], col="red",xlab="Iterations",ylab="Squared Error in dB", type="b", main="Agent with max. neighbours",pch=16)
